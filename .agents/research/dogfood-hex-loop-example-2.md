...
- **Date:** 2026-09-23 (re-run against `607a8ae`, the round-2 review fixes; I9 re-rendered after the `{branch}` amendment — `adr_0018` § Amendments — to name `"hex/plan-mirror-signing"`, the goal file's slug, and recounted)
...
— 2702/4000 chars (counted) · wrapper: /goal — native in Claude Code, Codex CLI, Cursor CLI; drop the prefix elsewhere
...
```text
$ python3 -c 'import sys; print(len(open(sys.argv[1], encoding="utf-8").read()))' .tmp/dogfood-r4/ex2.txt
2702
```
