...
| 5 | Deep-sweep gear via capability classes | **No new mechanism.** `researcher` × ≤12, two asynchronous waves, orchestrator-side dedup, batched under the existing cap with the existing batch-split disclosure. No coordinator, no nested spawn, no new class. **C-707** |
...
| **C-703** | **Dual question cadence.** *Inventory* questions (facts the user simply has) batch into one composite ask. *Design* questions are **strictly one per turn**, and every one carries an attached recommendation. Never a numbered list of design questions, never two design questions in one turn — the single-question law is the strongest finding in `discuss-skills-field.md`. | `hex-discuss/SKILL.md` |
...
| **C-705** | **Grill ruleset — four rules, all four normative.** (a) **Rebuttal gate**: categorize user pushback as *new evidence* (update the position and state what changed) or *repeated opinion* (hold, and restate the evidence) — never concede on repetition alone. (b) **Anti-theater**: never manufacture an objection; on agreement about a decision-relevant point, name the strongest remaining counter-argument **once** and move on. (c) **Scoped elicitation**: pick **≤2** fitting techniques per thread (premortem, inversion, first-principles, force-rank) — never present a catalog. (d) **Researcher blindness**: a research prompt states the question neutrally and **never reveals which side the user or the orchestrator favors**. | `hex-discuss/SKILL.md` |
| **C-706** | **Default research gear.** At most **3 concurrent** `researcher` spawns, model class `fast-balanced` by default — the researcher's cell at every tier (`models.md`), so hex-discuss never escalates *on its own judgment*. It is **not** exempt from `models.md` rule 1: `models.overrides` is a frozen v1 config key that applies at every tier, so a project can pin `researcher` above its cell and hex-discuss will run it there. That escalation is disclosed exactly as everywhere else — via the resolved-literal-model line, printed once on the first spawn of the role (C-712). A spawn is permitted only on a **decision-relevant disputed fact** — never on an opinion, never on a question the repo answers (read it instead). Researchers are **never on the critical path**: the conversation continues while they run, and a result arrives as a one-line aside (C-712). A researcher that **returns nothing useful** is folded in with no aside — there is nothing to report. A researcher that **fails to return at all** is a different event and is surfaced **once**, as a one-line transport note; a dead worker is never normalized into "no result found." | `hex-discuss/SKILL.md`; roles from `workers.md` |
| **C-707** | **Deep-sweep gear — no new mechanism.** The on-demand sweep is `researcher` spawns run in **two waves** — breadth (discover), orchestrator-side dedup, then depth (analyze) — with a **hard total cap of 12** (`max-workers` caps concurrency, not the total) and each wave bounded by the existing effective concurrency cap `min(8, limits.max-workers)`. **A demand above 12 truncates to 12, announced once.** A wave exceeding the cap runs in sequential batches per `protocol.md` § Worker coordination and **announces the batch split with the cap's source**. No `coordinator`, no nested spawning, no new role, no new capability class: the dedup step is the orchestrator's own synthesis duty, which `protocol.md` already assigns it. Findings longer than a paragraph persist per lane as research artifacts. **The two waves are asynchronous**: wave 2 launches when wave-1 results arrive, not at a barrier the conversation waits on — the sweep never blocks a conversational turn, so the hard Latency NFR holds unchanged. Dedup is therefore **incremental, not global**: each arriving wave-1 result is deduplicated against the depth lanes already dispatched (first seen wins) — a deliberate trade of globally-optimal dedup for a conversation that never waits, with a duplicate depth lane at the margin bounded by the hard cap. The sweep is offered as a chips moment (quick check / deep sweep / skip) when a topic looks sweep-worthy, never entered silently — and under `Degraded: inline workers` there is no background wave to buy, so the sweep **re-caps to the quick check's 3** and the chip text drops "background". That moment is a **bounded, user-initiated spend confirmation, not an approval gate**: nothing strands if the user declines, no state advances if they accept, and it exists only because a 12-worker spend mid-conversation must never be silent. **Not YAGNI:** the demand is proven, not speculative — this design's own dogfood discussion ran a user-requested two-wave, 12-agent sweep mid-conversation on 2026-08-28, and the per-lane research artifacts it produced are the ones this ADR cites. | `hex-discuss/SKILL.md` |
...
| ID | Scenario |
|---|---|
| **S-701** | A three-turn question ("should this be a flag or a subcommand?") is answered and drains inline. Entry wrote the header-only stub (C-715); the inline drain **deletes it**, so the run leaves **no discussion artifact** — C-709's no-obligation clause holds and the net file count is zero. |
...
| **S-711** | A deep sweep resolves to 11 researchers **in total across the two waves** — under the hard cap of 12. Batching is a *concurrency* split, not a wave split: a demand of 11 **within one wave** runs as batches of 8 then 3 under the effective cap `min(8, limits.max-workers)`, and the split prints once with the cap's source. The waves themselves stay the breadth→depth division — wave 2 dispatches each deduped depth lane as its wave-1 result arrives, blocking no conversational turn. No coordinator is spawned and no nested spawn occurs. **After the sweep a research artifact exists per lane** for every finding longer than a paragraph (C-707). |
...
|---|---|
| Scalability | Bounded by construction — ≤3 concurrent researchers by default, ≤12 total in a sweep, both inside the existing `min(8, max-workers)` cap counted recursively. No new fan-out mechanism and no new recursion level. |
...
- [ ] `hex-discuss` runs end-to-end with the rule file deleted, exercising
      **C-701…C-717 and C-721** (proves C-719 — C-718…C-720 are the rule's
      ...
      reference ships with the skill and is exercised regardless). After a
...
- [ ] A dogfood discussion exercises mode entry, one background research
      pass, a chips moment, the restate-gate refusing a soft confirmation,
      and a drain to plan.
...
- [ ] The **quiet announce form** holds (C-712): a dogfood turn asserts a
      research aside renders as **one line, never a block**, and S-710 is
      ...
      run identically, the only loss being that the stance may lapse after a
      compaction.
- [ ] The three announce forms that no other item exercises each render as
      **one line, once** (C-712): the entry-write disclosure
      `— discussion notes: <path>` on the run that writes the stub; the
...
- [ ] **Sweep cap boundary** (C-707): a demand above 12 **truncates to 12,
      announced once**; and under `Degraded: inline workers` the sweep
      **re-caps to the quick check's 3** with the chip text dropping
      "background".
