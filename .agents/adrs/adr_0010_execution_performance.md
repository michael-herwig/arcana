...
- **Site-table check:** for each row marked "no", the quoted sentence is still
  ...
  - `grep -rn "after every merge" hex/` — **zero `protocol.md` hits**, because
    ...
    **`hex-execute/SKILL.md`'s *"Recompute the ready-set after every merge"*,
    ...
    correct hit that must stay**, `hex/CHANGELOG.md`'s `0.2.0` Notes clause,
...
| 2026-08-31 | Erratum — the release shipped as **`0.2.0`**, not the `0.4.0` C-919 names. The authored-but-never-published `0.2.0` (discussion mode) and `0.3.0` (finalize phase) sections were consolidated with this ADR's release into one `## [0.2.0]` CHANGELOG section at publish time — `0.1.1` was the last published version, so the next tag is `v0.2.0`. C-919's mentions of `0.4.0` (its cell, § Decision step 7, § Considered Options, the plan's release checklist) are historical and read as the consolidated `0.2.0`; § Validation's live CHANGELOG reference is updated. Owner decision at release. *plan_adr_0010 release, 2026-08-31; Status: Accepted, unchanged* |
| 2026-09-05 | **Superseded by pointer — C-905's `Verify` cell, bytes intact.** The cell's "sets the WP's **merge gate** (C-901) and nothing else" widens by exactly one adjacent gate: under the Wave 0 quick-wins plan's C-924 the cell sets **one verification budget for one merge boundary** — that WP's merge gate *and* the Review-Fix-Loop exit gate that immediately precedes it, and nothing beyond those two. This cell's text is left as written; the amendment is recorded in `hex/DESIGN.md` round 14 (2026-09-05), per round 11's convention, and the clause round 12 defends — "the final gate is unchanged, mandatory, and un-lowerable by any per-WP budget" — is preserved verbatim by C-926. The Implement-phase gate is deliberately not coupled to the cell. Column direction, floor and defaults are unchanged. *plan_wave0_quick_wins, 2026-09-05; Status: Accepted, unchanged* |
| 2026-09-05 | **Superseded by pointer — C-901's Implement-phase scope statement, bytes intact.** "**The Implement-phase verification is not this gate and does not change**" no longer holds: under the Wave 0 quick-wins plan's C-925 the Implement gate becomes the **scoped check, unconditionally at every tier**, replacing the three shipped phrasings ("for changed files", "each work package's changed files", "across the whole workspace") with one vocabulary. The superseded sentence was a scope statement about this ADR's own delta, not a policy; its stated reason (phase 3 "already reads for changed files") is preserved and sharpened rather than discarded, and the leaf-under-coordinator carve-out is unchanged. Tier `high` gives up its only pre-merge whole-workspace proof; the backstop is named in C-925 — checkpoint trigger (ii), C-904's bounded bisection across at most three merges, trigger (i) at a coordinator join, and the terminal final gate. This cell's text is left as written; the amendment is recorded in `hex/DESIGN.md` round 14 (2026-09-05), per round 11's convention. Nothing else in C-901 changes — the merge-gate triggers, the override paths and the sub-WP rule are untouched. *plan_wave0_quick_wins, 2026-09-05; Status: Accepted, unchanged* |
