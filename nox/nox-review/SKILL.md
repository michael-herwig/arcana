...
| `counts:` | `neutralized`, `omitted` and `filtered`, each as `N of M`. `neutralized` is the instruction, hook and agent-config files removed by name. `omitted` is the untracked files the reviewer was not shown. `filtered` is the entries dropped by their *mode* — symlinks and submodules, each listed with its target — and never a count of instruction files. `M` above `N` means the enumeration was capped, never that the remainder was not there |
