...
auth status — from 2.1.281, the review streams and the 401 ladder from 2.1.260,
...
def _probe_runner(
    *,
    version_lines=("2.1.281 (Claude Code)",),
    version_exit: int = 0,
    auth_text: str | None = None,
    auth_exit: int = 0,
) -> FakeRunner:
    auth = _fixture("auth-status-2.1.281.json") if auth_text is None else auth_text
...
def test_no_evidence_flag_has_a_second_spelling_the_evidence_run_omits():
    ...
    groups = [
        ...
        for match in ALIAS_LINE.finditer(_fixture("help-2.1.281.txt"))
    ]
...
def test_parse_version_reads_the_dotted_release_out_of_the_recorded_line():
    ...
    assert parse_version(_fixture("version-2.1.281.txt")) == VERIFIED_AGAINST
...
def test_logged_out_reads_exactly_one_field_of_the_recorded_auth_object():
    ...
    assert logged_out(_fixture("auth-status-2.1.281.json")) is False
...
def test_an_advisory_line_ahead_of_the_auth_object_does_not_defeat_the_preflight():
    ...
    assert logged_out(advisory + _fixture("auth-status-2.1.281.json")) is False
...
def test_a_probe_whose_output_arrives_in_chunks_is_read_whole_before_it_is_reaped(tmp_path):
    ...
    runner = FakeRunner(
        TrickleProcess(["2.1.281 (Claude Code)\n"]),
