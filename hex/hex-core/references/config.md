...
| `perspectives.always` | ✅ | list of rule objects | `[]` | Adds a perspective: a checklist item in the `L2` aggregate seat's brief for `/hex-execute`, a seat in `/hex-review`'s `L3` panel and in `/hex-plan`'s and `/hex-architect`'s artifact panels ([`loop.md`](loop.md#review-by-join-level)). See [Perspectives](#perspectives). |
...
```yaml
# hex config, vocabulary v3. Unknown keys warn once and are ignored.
